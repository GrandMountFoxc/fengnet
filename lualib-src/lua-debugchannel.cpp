...
static struct channel *
channel_connect(channel *c) {
	struct channel * ret = NULL;
	lock_guard<SpinLock> lock(c->lock);
	if (c->ref == 1) {
		++c->ref;
		ret = c;
	}
...
channel_release(struct channel *c) {
	unique_lock<SpinLock> lock(c->lock);
	--c->ref;
	if (c->ref > 0) {
		lock.unlock();
		return c;
...
new_channel(lua_State *L, struct channel *c) {
	...
	struct channel_box * cb = (struct channel_box *)lua_newuserdatauv(L, sizeof(*cb), 0);
	cb->c = c;
...
LUAMOD_API int
luaopen_fengnet_debugchannel(lua_State *L) {
	luaL_Reg l[] = {
