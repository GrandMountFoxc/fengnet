...
static int
lclear(lua_State *L) {
	struct queue * q = static_cast<queue*>(lua_touserdata(L, 1));
	if (q == NULL) {
		return 0;
	}
	int i;
...
get_queue(lua_State *L) {
	struct queue *q = static_cast<queue*>(lua_touserdata(L,1));
	if (q == NULL) {
		q = static_cast<queue*>(lua_newuserdatauv(L, sizeof(struct queue), 0));
		q->cap = QUEUESIZE;
		q->head = 0;
		q->tail = 0;
		int i;
...
expand_queue(lua_State *L, struct queue *q) {
	struct queue *nq = static_cast<queue*>(lua_newuserdatauv(L, sizeof(struct queue) + q->cap * sizeof(struct netpack), 0));
...
save_uncomplete(lua_State *L, int fd) {
	struct queue *q = get_queue(L);
	int h = hash_fd(fd);
	struct uncomplete * uc = static_cast<uncomplete*>(malloc(sizeof(struct uncomplete)));
...
static int
lfilter(lua_State *L) {
	struct fengnet_socket_message *message = static_cast<fengnet_socket_message*>(lua_touserdata(L,2));
...
lpack(lua_State *L) {
	...
	uint8_t * buffer = static_cast<uint8_t*>(malloc(len + 2));
...
LUAMOD_API int
luaopen_fengnet_netpack(lua_State *L) {
	luaL_checkversion(L);
	luaL_Reg l[] = {
		{ "pop", lpop },
		{ "pack", lpack },
