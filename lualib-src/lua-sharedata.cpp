...
convert_stringmap(struct context *ctx, struct table *tbl) {
	...
	lua_pushvalue(L, 1);
	struct state * s = (struct state *)lua_newuserdatauv(L, sizeof(*s), 1);
	s->dirty = 0;
	s->ref = 0;
	s->root = tbl;
	lua_replace(L, 1);
...
lboxconf(lua_State *L) {
	...
	struct ctrl * c = (struct ctrl *)lua_newuserdatauv(L, sizeof(*c), 1);
	c->root = tbl;
	c->update = NULL;
...
lgetref(lua_State *L) {
	struct table *tbl = get_table(L,1);
	struct state * s = (struct state *)lua_touserdata(tbl->L, 1);
	lua_pushinteger(L , atomic_load(&s->ref));
...
LUAMOD_API int
luaopen_fengnet_sharedata_core(lua_State *L) {
	luaL_Reg l[] = {
		// used by host
		{ "new", lnewconf },
		{ "delete", ldeleteconf },
