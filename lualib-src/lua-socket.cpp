...
lfreepool(lua_State *L) {
	struct buffer_node * pool = static_cast<buffer_node*>(lua_touserdata(L, 1));
...
lnewpool(lua_State *L, int sz) {
	struct buffer_node * pool = static_cast<buffer_node*>(lua_newuserdatauv(L, sizeof(struct buffer_node) * sz, 0));
	int i;
	for (i=0;i<sz;i++) {
		pool[i].msg = NULL;
		pool[i].sz = 0;
...
lnewbuffer(lua_State *L) {
	struct socket_buffer * sb = static_cast<socket_buffer*>(lua_newuserdatauv(L, sizeof(*sb), 0));
	sb->size = 0;
	sb->offset = 0;
	sb->head = NULL;
	sb->tail = NULL;
...
lpushbuffer(lua_State *L) {
	struct socket_buffer *sb = static_cast<socket_buffer*>(lua_touserdata(L,1));
	if (sb == NULL) {
		return luaL_error(L, "need buffer object at param 1");
	}
	char * msg = static_cast<char*>(lua_touserdata(L,3));
	if (msg == NULL) {
	...
	struct buffer_node * free_node = static_cast<buffer_node*>(lua_touserdata(L,-1));	// sb poolt msg size free_node
	lua_pop(L,1);
	if (free_node == NULL) {
		...
		lnewpool(L, size);	
		free_node = static_cast<buffer_node*>(lua_touserdata(L,-1));
		lua_rawseti(L, pool_index, tsz+1);
		if (tsz > POOL_SIZE_WARNING) {
			Fengnet::inst->fengnet_error(NULL, "Too many socket pool (%d)", tsz);
		}
	}
...
return_free_node(lua_State *L, int pool, struct socket_buffer *sb) {
	...
	free_node->next = static_cast<buffer_node*>(lua_touserdata(L,-1));
	lua_pop(L,1);
	free(free_node->msg);
	free_node->msg = NULL;
...
static int
lunpack(lua_State *L) {
	struct fengnet_socket_message *message = static_cast<fengnet_socket_message*>(lua_touserdata(L,1));
	...
	if (message->type == FENGNET_SOCKET_TYPE_UDP) {
		int addrsz = 0;
		const char * addrstring = FengnetSocket::socketInst->fengnet_socket_udp_address(message, &addrsz);
		if (addrstring) {
...
lconnect(lua_State *L) {
	...
	struct fengnet_context * ctx = static_cast<fengnet_context*>(lua_touserdata(L, lua_upvalueindex(1)));
	int id = FengnetSocket::socketInst->fengnet_socket_connect(ctx, host, port);
...
lshutdown(lua_State *L) {
	int id = luaL_checkinteger(L,1);
	struct fengnet_context * ctx = static_cast<fengnet_context*>(lua_touserdata(L, lua_upvalueindex(1)));
	FengnetSocket::socketInst->fengnet_socket_shutdown(ctx, id);
	return 0;
}
...
llisten(lua_State *L) {
	...
	struct fengnet_context * ctx = static_cast<fengnet_context*>(lua_touserdata(L, lua_upvalueindex(1)));
	int id = FengnetSocket::socketInst->fengnet_socket_listen(ctx, host,port,backlog);
	if (id < 0) {
...
lsend(lua_State *L) {
	...
	buf.id = id;
	get_buffer(L, 2, &buf);
	int err = FengnetSocket::socketInst->fengnet_socket_sendbuffer(ctx, &buf);
...
lsendlow(lua_State *L) {
	...
	buf.id = id;
	get_buffer(L, 2, &buf);
	int err = FengnetSocket::socketInst->fengnet_socket_sendbuffer_lowpriority(ctx, &buf);
	lua_pushboolean(L, !err);
	return 1;
}
...
lbind(lua_State *L) {
	...
	int id = FengnetSocket::socketInst->fengnet_socket_bind(ctx,fd);
	lua_pushinteger(L,id);
	return 1;
}
...
lstart(lua_State *L) {
	struct fengnet_context * ctx = static_cast<fengnet_context*>(lua_touserdata(L, lua_upvalueindex(1)));
	int id = luaL_checkinteger(L, 1);
	FengnetSocket::socketInst->fengnet_socket_start(ctx,id);
	return 0;
}
...
lpause(lua_State *L) {
	struct fengnet_context * ctx = static_cast<fengnet_context*>(lua_touserdata(L, lua_upvalueindex(1)));
	int id = luaL_checkinteger(L, 1);
	FengnetSocket::socketInst->fengnet_socket_pause(ctx,id);
	return 0;
}
...
lnodelay(lua_State *L) {
	struct fengnet_context * ctx = static_cast<fengnet_context*>(lua_touserdata(L, lua_upvalueindex(1)));
	int id = luaL_checkinteger(L, 1);
	FengnetSocket::socketInst->fengnet_socket_nodelay(ctx,id);
	return 0;
}
...
ludp(lua_State *L) {
	...
	int id = FengnetSocket::socketInst->fengnet_socket_udp(ctx, host, port);
	if (id < 0) {
...
ludp_connect(lua_State *L) {
	...
	if (FengnetSocket::socketInst->fengnet_socket_udp_connect(ctx, id, host, port)) {
...
ludp_send(lua_State *L) {
	...
	buf.id = id;
	get_buffer(L, 3, &buf);
	int err = FengnetSocket::socketInst->fengnet_socket_udp_sendbuffer(ctx, address, &buf);
...
linfo(lua_State *L) {
	lua_newtable(L);
	struct socket_info * si = FengnetSocket::socketInst->fengnet_socket_info();
	...
	FengnetSocket::socketInst->socket_info_release(si);
	return 1;
}
...
LUAMOD_API int
luaopen_fengnet_socketdriver(lua_State *L) {
	luaL_checkversion(L);
	luaL_Reg l[] = {
		{ "buffer", lnewbuffer },
		{ "push", lpushbuffer },
