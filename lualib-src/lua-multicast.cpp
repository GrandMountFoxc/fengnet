...
pack(lua_State *L, void *data, size_t size) {
	struct mc_package * pack = (struct mc_package *)malloc(sizeof(struct mc_package));
	pack->reference = 0;
	pack->size = (uint32_t)size;
	pack->data = data;
	struct mc_package ** ret = (struct mc_package **)malloc(sizeof(*ret));
	*ret = pack;
...
mc_closelocal(lua_State *L) {
	...
	int ref = pack->reference.fetch_sub(1);
	if (ref <= 0) {
		free(pack->data);
		free(pack);
		if (ref < 0) {
...
LUAMOD_API int
luaopen_fengnet_multicast_core(lua_State *L) {
	luaL_Reg l[] = {
		{ "pack", mc_packlocal },
