...
create_proxy(lua_State *L, const void *data, int index) {
	...
	struct proxy * p = (struct proxy *)lua_newuserdatauv(L, sizeof(struct proxy), 0);
	...
	while (lua_next(L, t) != 0) {
		...
		if (lua_rawget(L, pt) == LUA_TUSERDATA) {
			...
			if (p->data == data) {
				...
				const struct table * newt = gettable((const document*)newdata, p->index);
				lua_pop(L, 1);
				// pointer, table
				clear_table(L);
				lua_pushvalue(L, lua_upvalueindex(1));
...
LUAMOD_API int
luaopen_fengnet_datasheet_core(lua_State *L) {
	luaL_checkversion(L);
	luaL_Reg l[] = {
		{ "new", lnew },
		{ "update", lupdate },
