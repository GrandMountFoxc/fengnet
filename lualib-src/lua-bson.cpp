...
static atomic<unsigned long> oid_counter;

static void
init_oid_header() {
	if (atomic_load(&oid_counter)) {
		// already init
		return;
	}
	pid_t pid = getpid();
...
lobjectid(lua_State *L) {
	...
	if (lua_isstring(L,1)) {
        ...
        uint32_t id = oid_counter.fetch_add(1);
