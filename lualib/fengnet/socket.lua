local driver = require "fengnet.socketdriver"
local fengnet = require "fengnet"
local fengnet_core = require "libfengnet.core"
local assert = assert
...
local function pause_socket(s, size)
	...
	if size then
		fengnet.error(string.format("Pause socket (%d) size : %d" , s.id, size))
	else
		fengnet.error(string.format("Pause socket (%d)" , s.id))
	end
	driver.pause(s.id)
	s.pause = true
	fengnet.yield()	-- there are subsequent socket messages in mqueue, maybe.
end
...
local function suspend(s)
	...
	if s.pause then
		fengnet.error(string.format("Resume socket (%d)", s.id))
		driver.start(s.id)
		fengnet.wait(s.co)
		s.pause = nil
	else
	...
	if s.closing then
		fengnet.wakeup(s.closing)
	end
end

-- read fengnet_socket.h for these macro
-- SKYNET_SOCKET_TYPE_DATA = 1
socket_message[1] = function(id, size, data)
	local s = socket_pool[id]
	if s == nil then
		fengnet.error("socket: drop package from " .. id)
	...
	if rrt == "number" then
		...
		if s.buffer_limit and sz > s.buffer_limit then
			fengnet.error(string.format("socket buffer overflow: fd=%d size=%d", id , sz))
			driver.close(id)
			return
		end
		if rrt == "string" then
...
socket_message[5] = function(id, _, err)
	...
	if s == nil then
		driver.shutdown(id)
		fengnet.error("socket: error on unknown", id, err)
		return
	end
	if s.callback then
		fengnet.error("socket: accpet error:", err)
		return
	end
	if s.connected then
		fengnet.error("socket: error on", id, err)
...
socket_message[6] = function(id, size, data, address)
	...
	if s == nil or s.callback == nil then
		fengnet.error("socket: drop udp package from " .. id)
		driver.drop(data, size)
		return
	end
	local str = fengnet.tostring(data, size)
	fengnet_core.trash(data, size)
...
local function default_warning(id, size)
	...
	fengnet.error(string.format("WARNING: %d K bytes need to send out (fd = %d)", size, id))
end
