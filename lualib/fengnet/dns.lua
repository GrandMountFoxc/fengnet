...
local function resolve(content)
	if #content < DNS_HEADER_LEN then
		-- drop
		fengnet.error("Recv an invalid package when dns query")
		return
	end
	...
	if answers_ipv4 then
		CACHE[QTYPE.A][question.name] = { answers = answers_ipv4, ttl = fengnet.now() + ttl * 100 }
	end

	if answers_ipv6 then
		CACHE[QTYPE.AAAA][question.name] = { answers = answers_ipv6, ttl = fengnet.now() + ttl * 100 }
	end
	...
	if question.name ~= resp.name then
		fengnet.error("Recv an invalid name when dns query")
	end
...
local function connect_server()
	...
	dns_server.fd = fd
	fengnet.error(string.format("Udp server open %s:%s (%d)", dns_server.address, dns_server.port, fd))
end

local DNS_SERVER_RETIRE = 60 * 100
local function touch_server()
	dns_server.retire = fengnet.now()
	...
	local function check_alive()
		if fengnet.now() > dns_server.retire + DNS_SERVER_RETIRE then
			local fd = dns_server.fd
			if fd then
				dns_server.fd = nil
				socket.close(fd)
				fengnet.error(string.format("Udp server close %s:%s (%d)", dns_server.address, dns_server.port, fd))
			end
		else
			fengnet.timeout( 2 * DNS_SERVER_RETIRE, check_alive)
		end
	end
...
local function lookup_cache(name, qtype, ignorettl)
	...
	if result then
		if ignorettl or (result.ttl > fengnet.now()) then
...
local function suspend(tid, name, qtype)
	...
	fengnet.fork(function()
		...
		if req then
			-- cancel tid
			fengnet.error(string.format("DNS query %s timeout", name))
			request_pool[tid] = nil
			fengnet.wakeup(req.co)
		end
	end)
	fengnet.wait(req.co)
	request_pool[tid] = nil
