...
local injectrun = require "fengnet.injectcode"
local table = table
local debug = debug
...
local fengnet_suspend
local fengnet_resume
local prompt
local newline
...
local function remove_hook(dispatcher)
	...
	fengnet.error "Leave debug mode"
end
...
local ctx_fengnet = debug.getinfo(fengnet.start,"S").short_src	-- skip when enter this source file
...
function linehook(mode, line)
	...
	if mode ~= "line" then
		ctx.needupdate = true
		if mode ~= "return" then
			if ctx.next_mode or debug.getinfo(2,"S").short_src == ctx_fengnet then
				ctx.level = 1
		...
		if ctx.needupdate then
			...
			if ctx.filename == ctx_term then
				ctx_active[co] = nil
				sethook()
				change_prompt(string.format(":%08x>", fengnet.self()))
				return
			end
		end
...
local function watch_proto(protoname, cond)
	local proto = assert(replace_upvalue(fengnet.register_protocol, "proto"), "Can't find proto table")
...
function dbgcmd.s(co)
	local ctx = ctx_active[co]
	ctx.next_mode = false
	fengnet_suspend(co, fengnet_resume(co))
end
	...
	if func then
		local function idle()
			if raw_dispatcher then
			    fengnet.timeout(10,idle)	-- idle every 0.1s
			end
		end
		fengnet.timeout(0, idle)
	end
	return func
end
...
function M.start(import, fd, handle)
	...
	fengnet_suspend = import.suspend
	fengnet_resume = import.resume
	assert(raw_dispatcher == nil, "Already in debug mode")
	fengnet.error "Enter debug mode"
	local channel = debugchannel.connect(handle)
	raw_dispatcher = hook_dispatch(dispatcher, fengnet.response(), fd, channel)
end
