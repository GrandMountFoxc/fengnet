	...
	while self.__sock do
		...
		if ok and session then
			...
			if co then
				...
				self.__thread[session] = nil
				fengnet.error("socket: unknown session :", session)
			end
		else
...
local function pop_response(self)
	while true do
		...
		fengnet.wait(self.__wait_response)
	end
end
...
local function autoclose_cb(self, fd)
	...
	if self.__wait_response and sock and sock[1] == fd then
		-- closed by peer
		fengnet.error("socket closed by peer : ", self.__host, self.__port)
		close_channel_socket(self)
	end
end
...
local function push_response(self, response, co)
	if self.__response then
		...
		if self.__wait_response then
			fengnet.wakeup(self.__wait_response)
...
local function connect_once(self)
	...
	local function _next_addr()
		...
		if addr then
			fengnet.error("socket: connect to backup host", addr.host, addr.port)
		end
		return addr
	end
	...
	local function _connect_once(self, addr)
				...
				if id == self.__sock[1] then
					if size == 0 then
						...
						if not self.__overload then
							...
							fengnet.error(string.format("WARNING: %d K bytes need to send out (fd = %d %s:%s)", size, id, self.__host, self.__port))
						end
					end
				end
			end

			fengnet.fork(overload_trigger, fd, 0)
		...
		self.__dispatch_thread = fengnet.fork(function()
			if self.__sock then
		...
		if self.__auth then
			...
			if not ok then
				...
				if message ~= socket_error then
					...
					fengnet.error("socket: auth failed", message)
				end
			end
			self.__authcoroutine = false
			if ok then
...
local function try_connect(self , once)
	...
	while not self.__closed do
		...
		if ok then
			if not once then
				fengnet.error("socket: connect to", self.__host, self.__port)
			end
			return
		elseif once then
			return err
		else
			fengnet.error("socket: connect", err)
		end
		if t > 1000 then
			fengnet.error("socket: try to reconnect", self.__host, self.__port)
			fengnet.sleep(t)
			t = 0
		else
			fengnet.sleep(t)
...
local function check_connection(self)
	if self.__sock then
		...
		if socket.disconnected(self.__sock[1]) then
			-- closed by peer
			fengnet.error("socket: disconnect detected ", self.__host, self.__port)
			close_channel_socket(self)
...
local function block_connect(self, once)
	...
	if r == nil then
		fengnet.error(string.format("Connect to %s:%d failed (%s)", self.__host, self.__port, err))
		error(socket_error)
	else
		return r
	end
