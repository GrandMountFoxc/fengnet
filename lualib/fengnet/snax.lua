...
local fengnet_send = fengnet.send
local fengnet_call = fengnet.call
...
local function gen_req(type, handle)
	return setmetatable({} , {
		__index = function( t, k )
			...
			return function(...)
				return fengnet_call(handle, "snax", id, ...)
			end
		end })
end
...
function snax.rawnewservice(name, ...)
	...
	local handle = fengnet.newservice("snaxd", name)
	assert(handle_cache[handle] == nil)
	if t.system.init then
		fengnet.call(handle, "snax", t.system.init, ...)
	end
	return handle
end
...
function snax.uniqueservice(name, ...)
	local handle = assert(fengnet.call(".service", "lua", "LAUNCH", "snaxd", name, ...))
	return snax.bind(handle, name)
end

function snax.globalservice(name, ...)
	local handle = assert(fengnet.call(".service", "lua", "GLAUNCH", "snaxd", name, ...))
	return snax.bind(handle, name)
end

function snax.queryservice(name)
	local handle = assert(fengnet.call(".service", "lua", "QUERY", "snaxd", name))
	return snax.bind(handle, name)
end

function snax.queryglobal(name)
	local handle = assert(fengnet.call(".service", "lua", "GQUERY", "snaxd", name))
	return snax.bind(handle, name)
end

function snax.kill(obj, ...)
	local t = snax.interface(obj.type)
	fengnet_call(obj.handle, "snax", t.system.exit, ...)
end

function snax.self()
	return snax.bind(fengnet.self(), SERVICE_NAME)
end
...
function snax.hotfix(obj, source, ...)
	local t = snax.interface(obj.type)
	return test_result(fengnet_call(obj.handle, "snax", t.system.hotfix, source, ...))
end

function snax.printf(fmt, ...)
	fengnet.error(string.format(fmt, ...))
end

function snax.profile_info(obj)
	local t = snax.interface(obj.type)
	return fengnet_call(obj.handle, "snax", t.system.profile)
end
