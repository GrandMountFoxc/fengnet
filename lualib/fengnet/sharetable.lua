...
local core = require "fengnet.sharetable.core"
...
local function sharetable_service()
    ...
    function sharetable.queryall(source, filenamelist)
        ...
        fengnet.ret(fengnet.pack(ptrList))
    end
				...
				if ref and ref.refs[source] then
					...
					if ref.count == 0 then
						if files[ref.filename] ~= ref.matrix then
							...
							fengnet.error(string.format("Delete a version (%s) of %s", ptr, ref.filename))
							ref.matrix:close()
							matrix[ptr] = nil
						end
					end
	...
	fengnet.info_func(function()
		...
		local function address(refs)
			...
			for addr in pairs(refs) do
				table.insert(keys, fengnet.address(addr))
			end
			table.sort(keys)
...
function sharetable.loadfile(filename, ...)
	fengnet.call(sharetable.address, "lua", "loadfile", filename, ...)
end

function sharetable.loadstring(filename, source, ...)
	fengnet.call(sharetable.address, "lua", "loadstring", filename, source, ...)
end

function sharetable.loadtable(filename, tbl)
	assert(type(tbl) == "table")
	fengnet.call(sharetable.address, "lua", "loadtable", filename, fengnet.pack(tbl))
end


local RECORD = {}
function sharetable.query(filename)
	local newptr = fengnet.call(sharetable.address, "lua", "query", filename)
	if newptr then
...
function sharetable.queryall(filenamelist)
    local list, t, map = {}
    local ptrList = fengnet.call(sharetable.address, "lua", "queryall", filenamelist)
