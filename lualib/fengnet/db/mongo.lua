...
require "fengnet.socket"

local socketchannel	= require "fengnet.socketchannel"
local fengnet = require "fengnet"
local driver = require "fengnet.mongo.driver"
...
function auth_method:auth_scram_sha1(username,password)
	...
	if not string.sub(rnonce, 1, 12) == nonce then
		fengnet.error("Server returned an invalid nonce.")
		return false
	end
	...
	if parsed_t['v'] ~= server_sig then
		fengnet.error("Server returned an invalid signature.")
		return false
	end
	if not r.done then
		...
		if not r.done then
			fengnet.error("SASL conversation failed to complete.")
			return false
		end
	end
	return true
