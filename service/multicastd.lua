...
local datacenter = require "fengnet.datacenter"

local harbor_id = fengnet.harbor(fengnet.self())
...
function command.DEL(source, c)
	...
	if node ~= harbor_id then
		fengnet.send(node_address[node], "lua", "DEL", c)
		return NORET
	end
	...
	if remote then
		for node in pairs(remote) do
			fengnet.send(node_address[node], "lua", "DELR", c)
		end
	end
	return NORET
end
...
local function remote_publish(node, channel, source, ...)
	fengnet.redirect(node_address[node], source, "multicast", channel, ...)
end

-- publish a message, for local node, use the message pointer (call mc.bind to add the reference)
-- for remote node, call remote_publish. (call mc.unpack and fengnet.tostring to convert message pointer to string)
local function publish(c , source, pack, size)
	...
	if remote then
		...
		local msg = fengnet.tostring(msg,sz)
		for node in pairs(remote) do
	...
	local msg = fengnet.tostring(pack, size)	-- copy (pack,size) to a string
	...
	for k in pairs(group) do
		...
		fengnet.redirect(k, source, "multicast", c , msg)
	end
end
...
function command.PUB(source, c, pack, size)
	assert(fengnet.harbor(source) == harbor_id)
	local node = c % 256
	if node ~= harbor_id then
		-- remote publish
...
function command.SUBR(source, c)
	local node = fengnet.harbor(source)
...
function command.SUB(source, c)
	...
	if node ~= harbor_id then
		...
		if channel[c] == nil then
			if fengnet.call(node_address[node], "lua", "SUBR", c) then
				return
			end
			if channel[c] == nil then
				-- double check, because fengnet.call whould yield, other SUB may occur.
				channel[c] = {}
				channel_n[c] = 0
			end
		end
	...
	if group[source] then
		...
		if channel_n[c] == 0 then
			...
			if node ~= harbor_id then
				...
				channel[c] = nil
				channel_n[c] = nil
				fengnet.send(node_address[node], "lua", "USUBR", c)
			end
		end
	end
	return NORET
...
fengnet.start(function()
	...
	local self = fengnet.self()
	local id = fengnet.harbor(self)
