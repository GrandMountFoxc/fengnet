...
push_queue_msg(struct harbor_msg_queue * queue, struct harbor_msg * m) {
	...
	if (((queue->tail + 1) % queue->size) == queue->head) {
		struct harbor_msg * new_buffer = (struct harbor_msg *)malloc(queue->size * 2 * sizeof(struct harbor_msg));
		int i;
...
new_queue() {
	struct harbor_msg_queue * queue = (struct harbor_msg_queue *)malloc(sizeof(*queue));
	queue->size = DEFAULT_QUEUE_SIZE;
	queue->head = 0;
	queue->tail = 0;
	queue->data = (harbor_msg *)malloc(DEFAULT_QUEUE_SIZE * sizeof(struct harbor_msg));
...
close_harbor(struct harbor *h, int id) {
	...
	if (s->fd) {
		FengnetSocket::socketInst->fengnet_socket_close(h->ctx, s->fd);
		s->fd = 0;
	}
	if (s->queue) {
...
report_harbor_down(struct harbor *h, int id) {
	...
	Fengnet::inst->fengnet_send(h->ctx, 0, h->slave, PTYPE_TEXT, 0, down, n);
}

extern "C" struct harbor *
harbor_create(void) {
	struct harbor * h = (struct harbor *)malloc(sizeof(*h));
	memset(h,0,sizeof(*h));
	h->map = hash_new();
	return h;
}
...
forward_local_messsage(struct harbor *h, void *msg, int sz) {
	...
	if (Fengnet::inst->fengnet_send(h->ctx, header.source, destination, type | PTYPE_TAG_DONTCOPY , (int)header.session, (void *)msg, sz-HEADER_COOKIE_LENGTH) < 0) {
		if (type != PTYPE_ERROR) {
			// don't need report error when type is error
			Fengnet::inst->fengnet_send(h->ctx, destination, header.source , PTYPE_ERROR, (int)header.session, NULL, 0);
		}
		Fengnet::inst->fengnet_error(h->ctx, "Unknown destination :%x from :%x type(%d)", destination, header.source, type);
	}
}

static void
send_remote(struct fengnet_context * ctx, int fd, const char * buffer, size_t sz, struct remote_message_header * cookie) {
	size_t sz_header = sz+sizeof(*cookie);
	if (sz_header > UINT32_MAX) {
		Fengnet::inst->fengnet_error(ctx, "remote message from :%08x to :%08x is too large.", cookie->source, cookie->destination);
		return;
	}
	...
	FengnetSocket::socketInst->fengnet_socket_sendbuffer(ctx, &tmp);
}
...
dispatch_name_queue(struct harbor *h, struct keyvalue * node) {
	...
	struct fengnet_context * context = h->ctx;
	...
	if (fd == 0) {
		if (s->status == STATUS_DOWN) {
			...
			Fengnet::inst->fengnet_error(context, "Drop message to %s (in harbor %d)",tmp,harbor_id);
		} else {
			if (s->queue == NULL) {
				s->queue = node->queue;
				node->queue = NULL;
			...
			if (harbor_id == (h->slave >> HANDLE_REMOTE_SHIFT)) {
				...
				while ((m = pop_queue(s->queue)) != NULL) {
					...
					Fengnet::inst->fengnet_send(context, m->header.source, handle , type | PTYPE_TAG_DONTCOPY, m->header.session, m->buffer, m->size);
				}
				release_queue(s->queue);
				s->queue = NULL;
			}
...
static void
push_socket_data(struct harbor *h, const struct fengnet_socket_message * message) {
	assert(message->type == FENGNET_SOCKET_TYPE_DATA);
	int fd = message->id;
	int i;
	int id = 0;
	struct slave * s = NULL;
	...
	if (s == NULL) {
		Fengnet::inst->fengnet_error(h->ctx, "Invalid socket fd (%d) data", fd);
		return;
	}
	...
	for (;;) {
		...
		case STATUS_HANDSHAKE: {
			...
			if (remote_id != id) {
				Fengnet::inst->fengnet_error(h->ctx, "Invalid shakehand id (%d) from fd = %d , harbor = %d", id, fd, remote_id);
				close_harbor(h,id);
				return;
			}
			++buffer;
		...
		case STATUS_HEADER: {
			...
			if (size < need) {
				...
				if (s->size[0] != 0) {
					Fengnet::inst->fengnet_error(h->ctx, "Message is too long from harbor %d", id);
					close_harbor(h,id);
					return;
				}
				s->length = s->size[1] << 16 | s->size[2] << 8 | s->size[3];
				s->read = 0;
				s->recv_buffer = (char*)malloc(s->length);
...
remote_send_handle(struct harbor *h, uint32_t source, uint32_t destination, int type, int session, const char * msg, size_t sz) {
	...
	if (harbor_id == h->id) {
		// local message
		Fengnet::inst->fengnet_send(context, source, destination , type | PTYPE_TAG_DONTCOPY, session, (void *)msg, sz);
		return 1;
	}
	...
	if (s->fd == 0 || s->status == STATUS_HANDSHAKE) {
		if (s->status == STATUS_DOWN) {
			...
			Fengnet::inst->fengnet_send(context, destination, source, PTYPE_ERROR, session, NULL, 0);
			Fengnet::inst->fengnet_error(context, "Drop message to harbor %d from %x to %x (session = %d, msgsz = %d)",harbor_id, source, destination,session,(int)sz);
		} else {
			if (s->queue == NULL) {
				s->queue = new_queue();
			}
...
remote_send_name(struct harbor *h, uint32_t source, const char name[GLOBALNAME_LENGTH], int type, int session, const char * msg, size_t sz) {
	...
	if (node->value == 0) {
		...
		Fengnet::inst->fengnet_send(h->ctx, 0, h->slave, PTYPE_TEXT, 0, query, strlen(query));
		return 1;
	} else {
...
handshake(struct harbor *h, int id) {
	...
	tmp.sz = 1;
	FengnetSocket::socketInst->fengnet_socket_sendbuffer(h->ctx, &tmp);
}
...
harbor_command(struct harbor * h, const char * msg, size_t sz, int session, uint32_t source) {
	...
	case 'N' : {
		if (s <=0 || s>= GLOBALNAME_LENGTH) {
			Fengnet::inst->fengnet_error(h->ctx, "Invalid global name %s", name);
			return;
		}
	...
	case 'A' : {
		...
		if (fd == 0 || id <= 0 || id>=REMOTE_MAX) {
			Fengnet::inst->fengnet_error(h->ctx, "Invalid command %c %s", msg[0], buffer);
			return;
		}
		struct slave * slave = &h->s[id];
		if (slave->fd != 0) {
			Fengnet::inst->fengnet_error(h->ctx, "Harbor %d alreay exist", id);
			return;
		}
		slave->fd = fd;

		FengnetSocket::socketInst->fengnet_socket_start(h->ctx, fd);
		handshake(h, id);
		if (msg[0] == 'S') {
	...
	default:
		Fengnet::inst->fengnet_error(h->ctx, "Unknown command %s", msg);
		return;
	}
}
...
static int
mainloop(struct fengnet_context * context, void * ud, int type, int session, uint32_t source, const void * msg, size_t sz) {
	struct harbor * h = (struct harbor *)ud;
	switch (type) {
	case PTYPE_SOCKET: {
		const struct fengnet_socket_message * message = (const struct fengnet_socket_message *)msg;
		switch(message->type) {
		case FENGNET_SOCKET_TYPE_DATA:
			push_socket_data(h, message);
			free(message->buffer);
			break;
		case FENGNET_SOCKET_TYPE_ERROR:
		case FENGNET_SOCKET_TYPE_CLOSE: {
			int id = harbor_id(h, message->id);
			if (id) {
				report_harbor_down(h,id);
			} else {
				Fengnet::inst->fengnet_error(context, "Unkown fd (%d) closed", message->id);
			}
			break;
		}
		case FENGNET_SOCKET_TYPE_CONNECT:
			// fd forward to this service
			break;
		case FENGNET_SOCKET_TYPE_WARNING: {
			int id = harbor_id(h, message->id);
			if (id) {
				Fengnet::inst->fengnet_error(context, "message havn't send to Harbor (%d) reach %d K", id, message->ud);
			}
			break;
		}
		default:
			Fengnet::inst->fengnet_error(context, "recv invalid socket message type %d", type);
			break;
		}
		return 0;
	}
	...
	default:
		Fengnet::inst->fengnet_error(context, "recv invalid message from %x,  type = %d", source, type);
		if (session != 0 && type != PTYPE_ERROR) {
			Fengnet::inst->fengnet_send(context,0,source,PTYPE_ERROR, session, NULL, 0);
		}
		return 0;
	}
}
...
extern "C" int
harbor_init(struct harbor *h, struct fengnet_context *ctx, const char * args) {
	h->ctx = ctx;
	int harbor_id = 0;
	uint32_t slave = 0;
	...
	Fengnet::inst->fengnet_callback(ctx, h, mainloop);
	FengnetHarbor::harborInst->fengnet_harbor_start(ctx);
