#ifndef fengnet_hashid_h
#define fengnet_hashid_h
...
hashid_init(struct hashid *hi, int max) {
	...
	hi->cap = max;
	hi->count = 0;
	hi->id = (hashid_node *)malloc(max * sizeof(struct hashid_node));
	for (i=0;i<max;i++) {
		hi->id[i].id = -1;
		hi->id[i].next = NULL;
	}
	hi->hash = (hashid_node **)malloc(hashcap * sizeof(struct hashid_node *));
